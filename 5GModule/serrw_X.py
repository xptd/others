...
def port_open():
    global ser
    global fd
    ser=serial.Serial('/dev/ttyUSB1',115200,rtscts=True,dsrdtr=True,timeout=5)
    ser.flushInput()
    if ser.isOpen():
        fd=open('/var/log/5gip','wr')
        os.chmod("/var/log/5gip",stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO )       
    else:
        print("open  /dev/ttyUSB1 failed!\n")
        os.system('echo `date`" serrw_X.py: [*] serial open failed" >>/var/log/5glog')
    ...
    #定义全部变量
        ...
        try:
            while True:
                    ...
                    fd.write('\n')
                    while True:
                            sendsim_count=sendsim_count + 1
                            if sendsim_count == 50:
                                os.system('echo `date`" : [*] serial send cardmode error">>/var/log/5glog')
                                sys.exit(1)
                            send("at^cardmode\r\n")
                            recdata=recv()
                            if recdata is None:
                    ...
                    fd.write('\n')  
                    while True:
                            senddis_count=senddis_count + 1
                            if senddis_count == 50:
                                os.system('echo `date`" : [*] serial send dis error">>/var/log/5glog')
                                sys.exit(1)
                            send("at^ndisdup=1,1,\"3gnet\"\r\n")
                            recdata=recv()
                            if recdata is None:
                                continue
                            if "OK" in recdata:
